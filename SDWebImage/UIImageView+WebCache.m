...
- (void)sd_setAnimationImagesWithURLs:(NSArray *)arrayOfURLs {
    ...
    for (NSURL *logoImageURL in arrayOfURLs) {
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:logoImageURL options:0 timings:SDDownloadTimings.new progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL, SDDownloadTimings *timings) {
            if (!wself) return;
            dispatch_main_sync_safe(^{
